...
class KoreaPage extends StatefulWidget {
  const KoreaPage({Key? key}) : super(key: key);

  @override
  State<KoreaPage> createState() => _LoginPageState();
}

class _LoginPageState extends State<KoreaPage> {
  var _feedbackText = '';
  ...
  void _handleclick(int i){
    ...
    switch(i){
      case 1:
        setState(() {
          _feedbackText = "$temp บาทเท่ากับ ${MyConverter.B2KR(temp)}วอน";
        });
        break;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          title: Text('แปลงสกุลวอน')),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            Text('แปลงจากสกุลเงินบาทเป็นสกุลเงินวอน',style: TextStyle(fontSize: 40)),
            Row(
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      _buildButton("แปลงเป็นเงินวอน",1),
                    ],
                  ),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text('อัตราแลกเปลี่ยน 1 บาท = 36.53 วอน',style: TextStyle(fontSize: 30),)
                    ],
                  ),
                ],
              ),
