...
class LaoPage extends StatefulWidget {
  const LaoPage({Key? key}) : super(key: key);

  @override
  State<LaoPage> createState() => _LoginPageState();
}

class _LoginPageState extends State<LaoPage> {
  var _feedbackText = '';
  ...
  void _handleclick(int i){
    ...
    switch(i){
      case 1:
        setState(() {
          _feedbackText = "$temp บาทเท่ากับ ${MyConverter.B2LAK(temp)}กีบ";
        });
        break;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          title: Text('แปลงสกุลเงินกีบ')),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
          children: [
            Text('แปลงจากสกุลเงินบาทเป็นสกุลเงินกีบ',style: TextStyle(fontSize: 40)),
            Row(
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      _buildButton("แปลงเป็นเงินกีบ",1),
                    ],
                  ),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text('อัตราแลกเปลี่ยน 1 บาท = 471.22 กีบ',style: TextStyle(fontSize: 30),)
                    ],
                  ),
                ],
              ),
